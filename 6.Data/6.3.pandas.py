...
def correct_state(row):
    if row['state'] != 0:
        ...
        if state:
            state_name = state[0]
            print u'state_name=\n',state_name
            a=' '.join(map(capital, state_name.split(' ')))
            print u'a=\n',a
            return a
    return row['state']
...
if __name__ == "__main__":
    ...
    # apply的使用
    print '==============apply的使用=========='
    data.apply(enum_row,axis=1) # axis=1表示行的方向axis=1
    ...
    # print process.extract('Mississipi', states)
    ...
    print 'All Columns:\n'
    print data.groupby('State Code').sum()#州缩写相同的那些行，对应元素相加
    print 'Short Columns:\n'
    print data[['State Code', 'Jan', 'Feb', 'Mar', 'total']].groupby('State Code').sum()
    #
    # # 写入文件
    data.to_excel('sales_result2.xls', sheet_name='Sheet1', index=False)#将data的index关闭
