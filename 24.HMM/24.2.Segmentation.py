...
def viterbi(pi, A, B, o):#给每个字做编码看它是0、1、2、3
    T = len(o)   # 观测序列
    ...
    for t in range(1, T):
        for i in range(4):
            delta[t][i] = delta[t-1][0] + A[0][i]#嵌套的循环从里到外看，把i看出具体的数比如1
            for j in range(1,4):#找最大的delta[t][i]aij,ln
                vj = delta[t-1][j] + A[j][i]
                if delta[t][i] < vj:
                    delta[t][i] = vj
                    pre[t][i] = j#记住前一个状态，以便回推出最佳路径
            delta[t][i] += B[i][ord(o[t])]#ln
    decode = [-1 for t in range(T)]  # 解码：回溯查找最大路径
    q = 0
    for i in range(1, 4):
        if delta[T-1][i] > delta[T-1][q]:
            q = i#找到使得delta[T-1][i]最大的那个状态
    decode[T-1] = q#最佳路径最后一步为该状态
    for t in range(T-2, -1, -1):
        q = pre[t+1][q]
        decode[t] = q
    return decode#把data里所有汉字的状态（词性）都表示出来了
...
if __name__ == "__main__":
    pi, A, B = load_train()#读取模型
    f = file(".\\novel.txt")
    data = f.read()[3:].decode('utf-8')
    f.close()
    decode = viterbi(pi, A, B, data)#用Viterbi算法对数据进行切分（分词）
    segment(data, decode)
