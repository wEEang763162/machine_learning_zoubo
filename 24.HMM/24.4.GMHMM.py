...
if __name__ == "__main__":
    ...
    n = 5   # 隐状态数目
    n_samples = 500
    pi = np.random.rand(n)#初始概率
    pi /= pi.sum()#各概率归一化，使得加和为1
    print '初始概率：', pi
    ...
    print means
    for i in range(n):
        means[i,:] /= np.sqrt(np.sum(means ** 2, axis=1))[i]#对每一行进行向量归一化
    print '均值：\n', means

    covars = np.empty((n, 3, 3))
    for i in range(n):
        # covars[i] = np.diag(np.random.randint(1, 5, size=2))
        covars[i] = np.diag(np.random.rand(3)*0.02+0.001) #避免取到0 # np.random.rand ∈[0,1)
    print '方差：\n', covars

    #造数据
    model = hmm.GaussianHMM(n_components=n, covariance_type='full')#跟EM算法的参数是一样的
    model.startprob_ = pi#根据参数进行采样
    model.transmat_ = A
    model.means_ = means#means和covars一起构成B发射矩阵
    model.covars_ = covars
    sample, labels = model.sample(n_samples=n_samples, random_state=0)

    # 估计参数
    model = hmm.GaussianHMM(n_components=n, covariance_type='full', n_iter=10)#EM算法为无监督学习，n_iter次里选最好的
    model.fit(sample)
    y = model.predict(sample)#baum welch算法算出模型参数，并算出每个样本（观测）背后的隐状态
    ...
    # 类别#无监督学习得到的类别的顺序跟样本本来的类别的顺序不一样
