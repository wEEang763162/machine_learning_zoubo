...
if __name__ == "__main__":
    ...
    models = [Pipeline([
        ...
        Pipeline([
            ...
            ('linear', ElasticNetCV(alphas=np.logspace(-3, 2, 10), l1_ratio=[.1, .5, .7, .9, .95, .99, 1],
                                    fit_intercept=False))])#不同alpha、l1_ratio值对预测性能的影响
    ]
    ...
    d_pool = np.arange(1, N, 1)  # 8阶，N=9
    m = d_pool.size
    clrs = []  # 颜色
    for c in np.linspace(16711680, 255, m):
        clrs.append('#%06x' % c)
    line_width = np.linspace(5, 2, m) #m=8
    ...
    for t in range(4):
        ...
        for i, d in enumerate(d_pool): #1阶到8阶
            model.set_params(poly__degree=d)
            model.fit(x, y.ravel())  #ravel=视图view，相当于引用
