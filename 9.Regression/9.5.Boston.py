...
if __name__ == "__main__":
    ...
    file_data = pd.read_csv('housing.data', header=None)# 返回行数
    # a = np.array([float(s) for s in str if s != ''])
    data = np.empty((len(file_data), 14))
    for i, d in enumerate(file_data.values):
        d = map(float, filter(not_empty, d[0].split(' ')))  #d[0].split(' ')会返回list
        data[i] = d
    x, y = np.split(data, (13, ), axis=1)#每行14列，前13列特征给x，最后一列标记（《西瓜书》）给y
    ...
    print u'截距：',linear.fit_intercept
